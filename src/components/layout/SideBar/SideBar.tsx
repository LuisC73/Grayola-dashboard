...
export function SideBar({ logo, items, button }: SideBarsProps) {
  ...
  return (
    <aside className="fixed max-h-screen w-full md:w-[200px] lg:w-[240px] h-fit md:h-full bg-custom bg-no-repeat border-r border-gray-300">
          ...
          {items?.map((item, index) => (
            <li key={index}>
              <Link
                href={item.href}
                title={item.title}
                className={`flex items-center gap-2 py-3 px-4 font-[family-name:var(--font-body)] text-sm text-black rounded-md hover:bg-tertiary/50 transition ease-out delay-150 ${
                  pathname === item.href ? 'bg-tertiary/30' : 'bg-transparent'
                }`}
              >
                <Icon {...item.icon} />
                {item.label}
      ...
      <div
        className={`w-4/5 h-screen absolute right-0 top-0 p-5 grid-rows-[auto_1fr] bg-gray-200 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-10 border-l border-gray-300 transition delay-150 ease-in grid md:hidden ${
            ...
            {items?.map((item, index) => (
              <li key={index}>
                <Link
                  href={item.href}
                  title={item.title}
                  className={`flex items-center gap-2 py-3 px-4 font-[family-name:var(--font-body)] text-sm text-black rounded-md hover:bg-tertiary/80 transition ease-in delay-150 ${
                    pathname === item.href ? 'bg-tertiary/50' : 'bg-transparent'
                  }`}
                >
                  <Icon {...item.icon} />
                  {item.label}
